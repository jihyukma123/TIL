...
- 정보를 많이 접하는데 한 번에 체득하지 못해서 활용을 못하고 결국 안본것과 마찬가지로 되어버리는 문제를 해소해주는 Agent

  - 예를 들어서, 내가 오늘 포프TV에서 `업무대화 중 소설 안쓰는 방법`이라는 영상을 봤어. 보면서 너무 좋은 내용이라서 나도 업무하면서 소통할 때 적용해야겠다고 생각했어. (Learned한 것을 써먹어보고자 하는 액션플랜)근데 과거에도 이런 지식/지혜는 많이 접했는데 실제로 적용까지 이어진건 생각보다 없단 말이지. 왜그럴까? 대부분 들을 때는 와 좋다 해봐야지 하지만 막상 살다보면 까먹기 십상이라서 그래.
  - 그래서 이런 페인포인트를 해결하기 위해서, 내가 접했던 지식/지혜 중에서 내가 써먹어보고 싶은 것들을 기억하고 상황에 맞춰서 추천해주는 Agent가 있으면 어떨까? 약간 personal 조언자 같은건데 이제 내가 기억하고 싶은 내용들 위주로 추천하는거지. DB를 계속 업데이트하고 피드백을 주고 받으면서 계속 커스텀화 수준이 높아지고 지혜도 깊어지지 않을까? 더 상호연결도 많이 되고!! 괜찮은 아이디어인 것 같음.

- 집중력 저하 개선을 도와주는 Agent
