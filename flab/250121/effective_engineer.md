...
저자는 어떤 형식으로건 TODOLIST를 반드시 만들어서 관리하라고 한다.

투두리스트는 2가지 속성을 갖춰야 하는데

- 업무를 정식으로 대표하는 목록이어야 하며
- 쉽게 접근 가능해야 한다.

하나의 MASTER 목록을 만들어라. SSOT를 만들어서 집중하면 정말 중요한 것, 즉 `우선순위 정하기`에 시간을 더 쓸 수 있음.

정량적으로 결정하려면 어렵다. 목록에 있는 일들 중에서 상대적으로 더 중요한 것과 덜 중요한 것을 정하는 방식이 더 쉬움.

현재 내가 하는 일이 목록에서 가장 레버리지가 높은 일인지 반복해서 물어라.

몇 가지 기준을 제시해주는데,

1. 직접적으로 가치를 생산하는 일에 집중하라.

출시한 제품, 확보한 사용자, 사업 지표 변동, 매출액 등이 그날 내가 얼마나 가치를 창출했는지 평가하는 지표가 되어야 한다.

예상되는 효과가 투자하는 노력에 비례하는지 확인해라. (이거는 웃긴 생각인데, 연봉협상보다도 매년 복리 수익을 몇 % 올려주는 건전한 포트폴리오를 만드는 것이 더 중요하다고도 말한다...ㅋㅋㅋㅋㅋ 여기서 내가 투자에 대해서도 공부를 더 미루면 안되겠다는 생각이 들다니...)

가장 적은 노력으로 가장 큰 가치를 생산하는 업무를 우선시하자.

모든 것을 하려고 하지 말고, 중요한 것에 집중하자.

2. 중요하지만 급하지 않은 일에 집중하라.

이게 좀 독특한 개념인데, (급하다/급하지 않다) \* (중요하다/중요하지 않다) 의 총 4가지 기준에서 급하고 중요한것보다 급하지 않지만 중요한 것들에 집중하라고 한다.

저자는 이 영역에 있는 일들이 장기적으로 봤을 때 가치가 크다고 주장함.

- 직업적 목표 정하기
- 강력한 인맥 구축
- 전문성 향상을 위해 책과 기사 읽기
- 생산성과 효율성을 높이는 새로운 습관 기르기
- 작업 흐름을 개선할 도구 만들기
- 유용한 추상화에 투자하기
- 인프라 꾸준히 확장하기
- 새로운 언어 배우기
...
이 영역에 있는 일들은 왜 급하지 않은가? 보통 기한이 없음. 이 점을 고려해서 접근하자.

3. 몰입을 고려해서 일정을 짜는게 도움이 된다.(프로그래머의 경우)

프로그래밍은 1시간 단위로 끊어서 진행하기 보다는 어느정도의 시간을 몰입해야 결과가 나오는 경우가 많다.

이 몰입의 시간 블록의 길이를 최대한 길게 유지하는 것이 좋다.

4. 동시에 진행하는 일을 최대한 제한하라.

음 여러가지를 한 번에 진행하면 집중도가 떨어질 수 밖에 없다...

아무래도 나도 지금 flab, 회사 과제, 회사 기술교육, 방통대 등 여러가지를 병행하는데 이걸 어떻게 좀 grouping할 수 있는 방법에 대해서 고민이 필요하겠다.

5. 자신만의 루틴이 필요하다.

우선순위를 정하는 것에 대한 나름대로의 루틴을 세워라.

저자는 아사나를 활용해서 `현재 우선순위`라는 프로젝트를 만들어서 우선순위를 관리한다고 한다.

한 주 단위로 끊어서 관리한다. 그리고 뽀모도로 25분을 하나의 단위로 해서 몇 뽀모도로가 소요되는지 예측해서 일정을 관리한다.

몇 가지 인상적인 내용들

- 뇌를 기억보관소가 아니라 프로세서로 활용해라. human error의 발생 가능성 자체를 줄여라. 이게 비단 개발뿐만 아니라

# 이번주에 읽은 부분에 대한 Action plans

- 레버리지 관점에서 지금 내가 진행하는 일들을 분류해보기.
  - 공부 관점에서, 지금 내가 제일 시간을 투자했을 때 나중에 어떤 공부가 제일 나한테 도움이 될지 나만의 기준을 세워봐야겠다.
  - 이와 연관해서, 직업적 목표 정하기
    - 직업적 목표가 있어야지 내가 이후에 어떤 방향으로 성장할지 알게 될 것 같음.
- 레버리지 질문 3가지 관점에서 내가 지금 하는 일들에 대해서 생각해보기.
- JIRA를 활용해서 한 주 단위로 계획을 짜고 우선순위를 선정해보기.
- 생산성 측면에서 Windsurf로 꾸준히 좀 개발해보기.
  - AI Coding 툴 사용하는 것에 대한 경험이 필요
- Intellij 잘 쓰는 사람 되기
  - Java/Spring Boot로 한동안은 개발을 할텐데, 문제가 뭔가 고민해봤는데 아무래도 Java 코드를 치는게 익숙치 않은 것도 있겠지만, Intellij 환경에 익숙하지 않은게 좀 큼. 그래서 날 잡아서 Intellij를 좀 잘 쓰는걸 익숙해지려고 함.
  - IDE
