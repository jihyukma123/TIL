...
오늘 나의 Agent가 리뷰해준 내용은 다음과 같다.

## 놓친 부분 및 깊이 있는 포인트

**1. SAML과 OAuth2.0의 결정적 차이: '목적'과 '대상'**

둘 사이의 가장 근본적인 차이는, '누구를 위한 기술인가'라는 점이라고 함.

- SAML: 조직이 주도권을 가짐. '우리 직원이니까 문열어줘' (사전에 구축된 강력한 서버 간 Trust 기반)
- OAuth 2.0: 사용자가 주도권을 가짐. '내 데이터를 저 앱이 쓰게 허락해줘' (Delegation, 권한 위임)
- 학습 내용 주에, OAuth가 SAML기반이라는 부분은 엔터프라이즈 환경에서 이야기고, B2C 아키텍처에서는 SAML없이 OIDC만 이용해서 인증/인가 처리하는 경우가 훨씬 많다는거.

**2. XML vs JSON: 단순히 '데이터 크기'만 가지고 overhead를 생각할게 아니다.**

자료 경량화가 얼마나 영향을 줄까?

- Parsing 비용
  - CPU 비용: XML Parser는 JSON 파서보다 무겁다고 함. (CPU 리소스 낭비)
- 개발 생산성
  - 매핑 복잡도: JSON은 현대 개발언어의 자료구조와 1:1로 매핑이 가능한데, XML은 트리 구조를 객체로 변환하는 과정이 번거로움. 개발 속도/버그 발생 가능성에 직결되어 개발 생산성이나 안정성에 영향을 준다. (AHA!)

**3. Stateful vs Stateless**

LDAP/SAML -> Stateful한 세션 기반 인증이 주였다고 함.

OAuth 2.0/JWT로 넘어오면서 Stateless 한 인증이 가능해졌음.

이 차이가 마이크로서비스(MSA) 환경에서 왜 결정적인지 고민해보면 좋다. (힌트: 서버를 수평 확장(Scale-out)할 때 세션 데이터는 어디에 있어야 할까?)

## 관점의 확장

**1. 경계 없는 보안(Zero Trust Security)**

과거에는 방화벽 안쪽은 안전하다고 가정했음.

근데 클라우드/원격근무 등 외부 환경과 함께 돌아가는 경우가 많아지면서, `내부망도 믿을 수 없다` 라는 패러다임이 등장했음. 이것이 `zero trust` 임.

사내망 접속 시에도 무조건 매 요청마다 '너 누구임? 권함 있음'을 검증하고, 이 때 OAuth 2.0/OIDC가 핵심 프로토콜로 사용됨.

**2. BFF(backend for frontend) 패턴**

토큰을 UI (브라우저)에 저장하는 건, 보안 취약점이 존재함.

그래서 최근에는 FE 대신, `BFF서버`가 토큰을 관리하고, 브라우저와는 HttpOnly Cookie로 세션을 맺는 방식이 많이 사용됨. 인증의 복잡성을 BFF 계층에 격리시키는거지.
